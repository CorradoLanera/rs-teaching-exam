---
title: "(Meta)data texting in `{ggplot2}`"
subtitle: "Formative assessments"
author: Corrado Lanera
output:
  learnr::tutorial:
    progressive: true
    allow_skip: false
runtime: shiny_prerendered
---

```{r setup, include=FALSE}
library(learnr)
library(parsons)
library(gradethis)
library(vdiffr)
library(tidyverse)
library(palmerpenguins)

knitr::opts_chunk$set(echo = FALSE)
gradethis_setup()


# https://github.com/rstudio-education/gradethis/issues/97
test_plots <- function(x, y) {
  fx <- fs::file_temp()
  fy <- fs::file_temp()
  # leave no footprint
  on.exit({
    unlink(c(fx, fy))
  })
  # write x
  vdiffr::write_svg(x, fx)
  # write y
  vdiffr::write_svg(y, fy)
  # compare
  identical(readLines(fx), readLines(fy))
}

```


## Step-by-step checks

### Data

```{r quiz-data}
quiz(caption = "Data-quiz",
  question("Textual information you found for each observation into your
           data are:",
           
    answer("data",
           correct = TRUE,
           message = random_praise()),
    
    answer("meta-data",
           message = "a meta-data is a property of the data you have,
           not the data itself"),
    
    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  
  question("When you want to add a single piece of text in a plot, it
           is:",
           
    answer("data",
           correct = TRUE,
           message = paste(random_praise(), "It is a single new data
             point. You can create a new data frame with a single row
             and pass this data to a suitable geoms, and continue to
             work in the standard basic way. On the other hand the
             `annotation()` layer is provided for exact this reason, and
             it is much much quicker and simpler to use!")),
    
    answer("meta-data",
           message = "a meta-data is a property of the data you have,
           not the data itself. In this case this piece of data are not
           a data included in your main dataset, but it still a (single)
           data."),
    
    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
```



### Aesthetics

```{r quiz-aes}
quiz(caption = "Aes-quiz",
  question("Suppose that in your data you have a varaible `var` 
           reporting textual information for each observation. 
           You would like to plot those directly into the plot.
           What aesthetics will you set for that variable 
           into the `aes()` call?",
           
    answer("label: `aes(x, y, label = var)`",
           correct = TRUE,
           message = random_praise()),
    
    answer("text: `aes(x, y, text = var)`",
           message = "geoms drawing text does not use the keyword
           `text` for that but `label`"),
    
    answer("colour: `aes(x, y, colour = var)`",
           message = "You can set `colour` to `var` if you want to draw
           the text in distinct colours (one colour for each distinct
           text) that do this...but you still miss to draw the text
           into the plot (and so you cannot see the colours) ;-)"),
    
    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
```

### {data-allow-skip=TRUE}
Check for yourself

```{r quiz-aes-code, exercise = TRUE}
penguins %>% 
  ggplot(aes(x = bill_length_mm, y = bill_depth_mm, ___ = species))
```





### Geom

```{r quiz-geom}
quiz(caption = "Geom-quiz",
  question("Which `geom_*`s are suitable for text?",
           
    answer("`geom_text()`",
           correct = TRUE,
           message = random_praise()),
    
    answer("`geom_label()`",
           correct = TRUE,
           message = paste(random_praise(), "`geom_label()` is like
           `geom_text()` but it draws a rectangle behind the text,
           making it easier to read")),
    
    answer("`geom_point()`",
           message = "Text will be used like points in a scatterplot,
           but they are not \"point\", they are text, sometimes called
           labels. If you call `geom_point()` you will obtain a simple
           point at every set of coordinates (x, y) in the data, without
           text or labels."),
    
    answer("geom_word()",
           message = "This geom does not exist in `{ggplot2}`
           currently."),
    
    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  
  
  question("If you want to draw ALL your textual data in blue How can
           you do?",
           
    answer("`geom_text(colour = \"blue\")`",
           correct = TRUE,
           message = random_praise()),
    
    answer("`geom_text(aes(colour = \"blue\"))`",
           message = "This, surprisingly, will create a new aesthetic
           map to draw the **data** \"blue\" (which is constant) to the
           color property. So this is not what you want, ie **set** a 
           custom property without any map from any data. Surpirisgly,
           the call will draw all your data in red :-)... if you are 
           curious why, take a look at chapter 12.4 of the
           [ggplot2 book](https://ggplot2-book.org/)"),
    
    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  
  
  question("What is the first argument of `annotate()`?",
           
    answer("A character string matching a (any) geom, like \"text\"",
           correct = TRUE,
           message = paste(random_praise(), "Yes, annotate can draw
             any explicitly provided data point(s) using any geoms.
             Particolarly useful but not limited to text and labels")),
    
    answer("A character string of the text annotation to plot",
           message = "`annotate()` is a wrapper for a single occurence
           of a geom. You need to firstly decide the geom to know which
           aesthetics to use. Next, you will set explicitely those
           aesthetics here (without mapping any variable to them)."),
    
    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
  
)
```

### {data-allow-skip=TRUE}
Check for yourself

```{r quiz-geom-code, exercise = TRUE}
penguins %>% 
  ggplot(aes(x = bill_length_mm, y = bill_depth_mm, label = species)) +
  geom____() + # try: colour = "blue" / aes(colour = "blue") ?
  annotate(___, x = 45, y = 17.5, label = "center",
           colour = "red", fill = "blue", size = 24)
```









### Scales

```{r quiz-scales}
quiz(caption = "Scale-quiz",
  question("What labelling functions you can use from the package
           `{scale}` (automatically imported with `{ggplot2}`) to 
           format ticks' labels for the axes",
           
    answer("`label_number()` for decimal format (e.g. 0.12, 1,234)",
           correct = TRUE,
           message = random_praise()),
    answer("`label_scientific()` for scientific notation (e.g. 1e05, 1.5e-02)",
           correct = TRUE,
           message = random_praise()),
    answer("`label_bytes()` to scale bytes into human friendly units (1 kb, 2 MB, etc)",
           correct = TRUE,
           message = random_praise()),
    answer("`label_dollar()` to label currencies (not only dollars)",
           correct = TRUE,
           message = random_praise()),
    answer("`label_percent()` for % to the number (2.5%, 50%, etc)",
           correct = TRUE,
           message = random_praise()),
    answer("`label_pvalue()` use \"<\" and \">\" for p-values close to 0 and 1",
           correct = TRUE,
           message = random_praise()),
    answer("`label_number_si()` use SI prefixes (2k, 1M, 5T etc)",
           correct = TRUE,
           message = random_praise()),
    answer("`label_wrong()` for answer suggestion (check this too: wrong!)",
           correct = TRUE,
           message = random_praise()),

        
    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
```

### {data-allow-skip=TRUE}
Check for yourself

```{r quiz-scales-code, exercise = TRUE}
penguins %>% 
  ggplot(aes(x = bill_length_mm, y = bill_depth_mm, label = species)) +
  geom_point() +
  scale_x_continuous(labels = scales::label____()) +
  scale_y_continuous(labels = scales::label____()) +
```









### Facets

```{r quiz-facet}
quiz(caption = "Facet-quiz",
  question("How can you personalize the titles of the facets in a grid
           plot?",
           
    answer("Using the `labeller` option of `facet_*()`",
           correct = TRUE,
           message = random_praise()),
    
    answer("Changing the content of the facetting varible(s) in the
           input data frame.",
           message = "This surely works, but use a `labeller`
           would be be much better. Take aa look to `?labeller` and
           http://bit.ly/change-facets-titles"),
    
    answer("Using the `strip.text` option of `theme()` and the
           `element_text()` function",
           message = "`theme()` can change only styles, not contents."),
    
    answer("d",
           message = ""),
    
    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
```

### {data-allow-skip=TRUE}
Check for yourself (you have some `hints` here)

```{r quiz-facet-code, exercise = TRUE}
penguins %>% 
  ggplot(aes(x = bill_length_mm, y = bill_depth_mm, label = species)) +
  geom_point() +
  facet_grid(island~sex, labeller = ___)
```


```{r quiz-facet-code-hint}
# try
facet_grid(island~sex, labeller = "label_both")

# or try
facet_grid(island~sex,
           labeller = labeller(sex = c(female = "F", male = "M")))

# or try
facet_grid(island~sex,
           labeller = labeller(island = label_both, sex = label_value))
```









### Theme

```{r quiz-theme}
quiz(caption = "Data-quiz",
  question("What can you change using `theme()`?",
           
    answer("Style",
           correct = TRUE,
           message = random_praise()),
    
    answer("Data",
           message = "`theme()` invole style only, no data change.
           using `theme()` you can only change _how_ things are viewed,
           not _what_!"),
    
    answer("Meta-data",
           message = "`theme()` invole style only, no meta-data change.
           using `theme()` you can only change _how_ things are viewed,
           not _what_!"),
    
    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  
  question("What would you use to change the style of a something
           related to the whole plot (like its title)",
           
    answer("`plot.*`",
           correct = TRUE,
           message = random_praise()),
    
    answer("`axis.*`"),
    answer("`legend.*`"),
    answer("`strip.*`"),
    answer("`facet.*`"),
    
    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  
  question("What would you use to change the style of a something
           related to the faceting's strips?",
           
    answer("`strip.*`",
           correct = TRUE,
           message = random_praise()),
    
    answer("`axis.*`"),
    answer("`legend.*`"),
    answer("`plot.*`"),
    answer("`facet.*`", message = "Unfortunately, `facet.*` is not how
           ggplot call facets when referring to them. But, they looks
           like strips..."),
    
    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  
  question("What can you use to change the style of the name of 
           both axes in the same way?",
           
    answer("`axis.title`",
           correct = TRUE,
           message = random_praise()),
    
    answer("set `axis.title.x`, `axis.title.y` in the same way",
           correct = TRUE,
           message = paste(random_praise(), "Anyway, this would not be 
             the best choice if the changes are the same")),
    answer("axis.text"),
    answer("set `axis.text.x`, `axis.text.y` in the same way"),

    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  
  question("What can you set using `element_text()`?",
           
    answer("Only style, like font face, family, size, angle, colour,
           etc",
           correct = TRUE,
           message = random_praise()),
    
    answer("Only textual element, like lables, and their position",
           message = paste(random_praise(), "`theme()` invole style
           only, no data nor meta-data change. Using `theme()` you can
           only change _how_ things are viewed, not _what_!")),
    answer("All the element involved in text data, like the elements
           displayed and their style",
           message = paste(random_praise(), "`theme()` invole style
           only, no data nor meta-data change. Using `theme()` you can
           only change _how_ things are viewed, not _what_!")),

    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
```









## Final Test



###

In the following, you will asked to create a plot similar to the one 
created in the class. We will reach the final result step by step! 

In the following you will be required to modify/update a purposed code
(similar to the one used in the class) accordingly to some instruction.

You can experiment your code as much as you want cliccking on the
`Run Code` button. You can click on `Start Over` to reset the code chunk
to original purposed one. You can click to `Hints` to come across some
hints if you need them. 

When you think you have reached the correct result, you can click on
`Submit Answer` to check it. 

> NOTE: if your answer is wrong, you can try again as much time as you 
  like. But you are required to pass a stage to go to the next one 
  (there is only one stage marked as "optional", which you can skip if
  you won't pass it).


Ready!? Let's go!



### Data and aesthetics

Map `body_mass_g` against `flipper_length_mm` showing the `species` at
   each data-point, coloured by `sex`

```{r final-data, exercise=TRUE, exercise.lines = 4}
penguins %>% 
  ggplot(aes(x = bill_length_mm, y = bill_depth_mm,
    label = sex, colour = species)) +
  geom_text(check_overlap = TRUE)
```

```{r final-data-check}
grade_result(pass_if(~{
  ref <- penguins %>% 
    ggplot(aes(x = body_mass_g, y = flipper_length_mm,
      label = species, colour = sex)) +
    geom_text(check_overlap = TRUE)

  test_plots(.result, ref)
  
}), incorrect = "Wrong answer.")
```


```{r final-data-hint-1}
# modify only (you do not need to add or remove lines of code)
aes(x = ___, y = ___, label = ___, colour = ___)
```

```{r final-data-hint-2}
# modify only (you do not need to add or remove lines of code)
aes(x = body_mass_g, y = flipper_length_mm,
    label = species, colour = sex)
```



### Annotations (_una tantum_ data)

Draw a label in the middle-left and in the middle-right part of
the plot reporting "Short" and "Long" respectively (use the "invard"
options as in class for a nicer (and graded correct) result)


```{r final-geom, exercise=TRUE, exercise.lines = 8}
penguins %>% 
  ggplot(aes(x = body_mass_g, y = flipper_length_mm,
    label = species, colour = sex)) +
  geom_text(check_overlap = TRUE)
```


```{r final-geom-check}
grade_result(pass_if(~{
  ref <- penguins %>% 
    ggplot(aes(x = body_mass_g, y = flipper_length_mm,
      label = species, colour = sex)) +
    geom_text(check_overlap = TRUE) +
    annotate("label", x = 4500, y = 170, label = "Short",
             hjust = "inward", vjust = "inward") +
    annotate("label", x = 4500, y = 230, label = "Long",
             vjust = "inward", hjust = "inward")


  test_plots(.result, ref)
  
}), incorrect = "Wrong answer.")
```

```{r final-geom-hint-1}
# use the following coordinate for the annotations
x = 4500, y = 170 # short
x = 4500, y = 230 # long
```

```{r final-geom-hint-2}
# use the following coordinate for the annotations
x = 4500, y = 170 # short
x = 4500, y = 230 # long

# the template for (one) annotation is the following one
annotate("label", x = ___, y = ___, label = ___,
         hjust = "inward", vjust = "inward")
```

```{r final-geom-hint-3}
# use the following coordinate for the annotations
x = 4500, y = 170 # short
x = 4500, y = 230 # long

# the template for (two) annotations is the following one
annotate("label", x = ___, y = ___, label = ___,
         hjust = "inward", vjust = "inward") +
annotate("label", x = ___, y = ___, label = ___,
         hjust = "inward", vjust = "inward")

# Remember to capitalize short ("Short") and long ("Long") for your
# label.
```


 

> Optional: Just for fun, try to remove all the `invard`s options from
your final (correct) result.


```{r final-geom-2, exercise=TRUE, exercise.lines = 6}
penguins %>% 
  ggplot(aes(x = body_mass_g, y = flipper_length_mm,
    label = species, colour = sex)) +
  geom_text(check_overlap = TRUE)
```


```{r final-geom-2-check}
grade_result(
  pass_if(~{
    ref <- penguins %>% 
      ggplot(aes(x = body_mass_g, y = flipper_length_mm,
        label = species, colour = sex)) +
      geom_text(check_overlap = TRUE) +
      annotate("label", x = 4500, y = 170, label = "Short") +
      annotate("label", x = 4500, y = 230, label = "Long")
  
      test_plots(.result, ref)
  
  }),
  fail_if(~TRUE, paste(
    random_encouragement(),
    "(Anyway, you can go on: this is not required to be correct!)"))
)
```





### Scales - axes (Meta-data + style)

Mark on the axes, weights with "g", and lengths with "mm" unit of
  measure; remove those units from the axes title, replace "_"s with 
  more readable " "s, Capitalize the first letter of the axes's titles.
  Write them in blue using sans-serif (`sans`) font family.

```{r final-scale, exercise=TRUE, exercise.lines = 13}
penguins %>% 
  ggplot(aes(x = body_mass_g, y = flipper_length_mm,
    label = species, colour = sex)) +
  geom_text(check_overlap = TRUE) +
  annotate("label", x = 4500, y = 170, label = "Short",
           hjust = "inward", vjust = "inward") +
  annotate("label", x = 4500, y = 230, label = "Long",
           vjust = "inward", hjust = "inward")

```


```{r final-scale-check}
grade_result(pass_if(~{
  ref <- penguins %>% 
    ggplot(aes(x = body_mass_g, y = flipper_length_mm,
      label = species, colour = sex)) +
    geom_text(check_overlap = TRUE) +
    annotate("label", x = 4500, y = 170, label = "Short",
             hjust = "inward", vjust = "inward") +
    annotate("label", x = 4500, y = 230, label = "Long",
             vjust = "inward", hjust = "inward") +
    scale_x_continuous("Body mass",
      labels = scales::label_comma(suffix = " g")) +
    scale_y_continuous("Flipper length",
      labels = scales::label_number(suffix = " mm")) +
    theme(axis.title = element_text(family = "sans", colour = "blue"))

  test_plots(.result, ref)
  
}), incorrect = "Wrong answer.")
```


```{r final-scale-hint-1}
# you can use
  scale_x_continuous(___, labels = scales::label_number(suffix = ___))
```

```{r final-scale-hint-2}
# you can use
  scale_x_continuous(___, labels = scales::label_number(suffix = ___))

# To style the axes' titles you can use the `them()` function
```

```{r final-scale-hint-3}
# you can use
  scale_x_continuous(___, labels = scales::label_number(suffix = ___))

# To style the axes' titles you can use the `theme()` function
theme(axis.___ = element_text(___ = "sans", colour = ___)
```



### Scale - legend (Meta-data + theme)

Capitalizethe legend's title (replacing "_"s with " "s) and texts, and
made the legend's title dark green. 
   
```{r final-legend, exercise=TRUE, exercise.lines = 17}
penguins %>% 
  ggplot(aes(x = body_mass_g, y = flipper_length_mm,
    label = species, colour = sex)) +
  geom_text(check_overlap = TRUE) +
  annotate("label", x = 4500, y = 170, label = "Short",
           hjust = "inward", vjust = "inward") +
  annotate("label", x = 4500, y = 230, label = "Long",
           vjust = "inward", hjust = "inward") +
  scale_x_continuous("Body mass",
    labels = scales::label_comma(suffix = " g")) +
  scale_y_continuous("Flipper length",
    labels = scales::label_comma(suffix = " mm")) +
  theme(axis.title = element_text(family = "sans", colour = "blue"))
```




```{r final-legend-check}
grade_result(pass_if(~{
  ref <- penguins %>% 
    ggplot(aes(x = body_mass_g, y = flipper_length_mm,
      label = species, colour = sex)) +
    geom_text(check_overlap = TRUE) +
    annotate("label", x = 4500, y = 170, label = "Short",
             hjust = "inward", vjust = "inward") +
    annotate("label", x = 4500, y = 230, label = "Long",
             vjust = "inward", hjust = "inward") +
    scale_x_continuous("Body mass",
      labels = scales::label_comma(suffix = " g")) +
    scale_y_continuous("Flipper length",
      labels = scales::label_comma(suffix = " mm")) +
    scale_colour_discrete("Sex",
      breaks = c("female", "male", NA),
      labels = c("Female", "Male", "NA")) +
    theme(axis.title = element_text(family = "sans", colour = "blue"),
          legend.title = element_text(colour = "darkgreen"))

  test_plots(.result, ref)
  
}), incorrect = "Wrong answer.")
```



```{r final-legend-hint-1}
# If you do not need to change colours, you should go with 
scale_colour_discrete() # and don't use the `value` option
```

```{r final-legend-hint-2}
# If you do not need to change colours, you should go with 
scale_colour_discrete() # and don't use the `value` option

scale_colour_discrete(
    breaks = c("female", "male", ___),
    labels = c(___, ___, ___))
```

```{r final-legend-hint-3}
# If you do not need to change colours, you should go with 
scale_colour_discrete() # and don't use the `value` option

# Remember to capitaliza the labels and that NA _is_ a missing, while
# "NA" is a string
scale_colour_discrete(
    breaks = c("female", "male", NA),
    labels = c(___, ___, ___))
```

```{r final-legend-hint-4}
# If you do not need to change colours, you should go with 
scale_colour_discrete() # and don't use the `value` option

# Remember to capitaliza the labels and that NA _is_ a missing, while
# "NA" is a string
scale_colour_discrete(
    breaks = c("female", "male", NA),
    labels = c(___, ___, "NA"))

# You can set the scale name (the legend's title in this case) both
# from the `scale_<AES>_<TYPE>()` function (first argument)
scale_colour_discrete(___,
    breaks = c("female", "male", NA),
    labels = c(___, ___, "NA"))
# OR you can set it using `labs()`, naming the corresponding aesthetics
labs(colour = ___) 
```

```{r final-legend-hint-5}
# If you do not need to change colours, you should go with 
scale_colour_discrete() # and don't use the `value` option

# Remember to capitaliza the labels and that NA _is_ a missing, while
# "NA" is a string
scale_colour_discrete(
    breaks = c("female", "male", NA),
    labels = c(___, ___, "NA"))

# You can set the scale name (the legend's title in this case) both
# from the `scale_<AES>_<TYPE>()` function (first argument)
scale_colour_discrete(___,
    breaks = c("female", "male", NA),
    labels = c(___, ___, "NA"))
# OR you can set it using `labs()`, naming the corresponding aesthetics
labs(colour = ___) 

# To make the legend's title dark green you can set it "darkgreen"
theme(legend.___ = element_text(___ = ___))
```

   
   
   
   
   
### Facets (Theme + very little of meta-data)

Divide the plot in a grid of plots. Display years into the columns and
islands into the rows, keeping the indication of the variable in the
rows and columns titles and write them both in **bold** and _italic_.


```{r final-facet, exercise=TRUE}
penguins %>% 
  ggplot(aes(x = body_mass_g, y = flipper_length_mm,
    label = species, colour = sex)) +
  geom_text(check_overlap = TRUE) +
  annotate("label", x = 4500, y = 170, label = "Short",
           hjust = "inward", vjust = "inward") +
  annotate("label", x = 4500, y = 230, label = "Long",
           vjust = "inward", hjust = "inward") +
  scale_x_continuous("Body mass",
    labels = scales::label_comma(suffix = " g")) +
  scale_y_continuous("Flipper length",
    labels = scales::label_comma(suffix = " mm")) +
  scale_colour_discrete("Sex",
    breaks = c("female", "male", NA),
    labels = c("Female", "Male", "NA")) +
  theme(axis.title = element_text(family = "sans", colour = "blue"),
        legend.title = element_text(colour = "darkgreen"))
```



```{r final-facet-check}
grade_result(pass_if(~{
  ref <- penguins %>% 
    ggplot(aes(x = body_mass_g, y = flipper_length_mm,
      label = species, colour = sex)) +
    geom_text(check_overlap = TRUE) +
    annotate("label", x = 4500, y = 170, label = "Short",
             hjust = "inward", vjust = "inward") +
    annotate("label", x = 4500, y = 230, label = "Long",
             vjust = "inward", hjust = "inward") +
    scale_x_continuous("Body mass",
      labels = scales::label_comma(suffix = " g")) +
    scale_y_continuous("Flipper length",
      labels = scales::label_comma(suffix = " mm")) +
    scale_colour_discrete("Sex",
      breaks = c("female", "male", NA),
      labels = c("Female", "Male", "NA")) +
    facet_grid(island~year, labeller = "label_both") +
    theme(axis.title = element_text(family = "sans", colour = "blue"),
          legend.title = element_text(colour = "darkgreen"),
          strip.text = element_text(face = "bold.italic"))

  test_plots(.result, ref)
  
}), incorrect = "Wrong answer.")
```


```{r final-facet-hint-1}
# Remember to keep the call to `theme()` as the last one
```

```{r final-facet-hint-2}
# Remember to keep the call to `theme()` as the last one

# For faceting, remember that `<LHS> ~ <RHS>` will display the <LHS> 
# variable across the grid rows (ie, every row is a distinct level
# of <LHS>), and `<RHS>` across the grid column
facet_grid(___~___)
```

```{r final-facet-hint-3}
# Remember to keep the call to `theme()` as the last one

# For faceting, remember that `<LHS> ~ <RHS>` will display the <LHS> 
# variable across the grid rows (ie, every row is a distinct level
# of <LHS>), and `<RHS>` across the grid column
facet_grid(___~___)

# You can use (the labeller is usefull if you do not 
# want standard label, see `?labeller` for more informations)
# to put the variable indication before the corresponding levels you can
# use `"label_both"`
facet_grid(___~___, labeller = ___)
```

```{r final-facet-hint-4}
# Remember to keep the call to `theme()` as the last one

# For faceting, remember that `<LHS> ~ <RHS>` will display the <LHS> 
# variable across the grid rows (ie, every row is a distinct level
# of <LHS>), and `<RHS>` across the grid column
facet_grid(___~___)

# You can use (the labeller is usefull if you do not 
# want standard label, see `?labeller` for more informations)
# to put the variable indication before the corresponding levels you can
# use `"label_both"`
facet_grid(___~___, labeller = ___)

# For bold and italic characters, you can use "bold.italic" where 
# needed in the theme
element_text(face = ___)
```

```{r final-facet-hint-5}
# Remember to keep the call to `theme()` as the last one

# For faceting, remember that `<LHS> ~ <RHS>` will display the <LHS> 
# variable across the grid rows (ie, every row is a distinct level
# of <LHS>), and `<RHS>` across the grid column
facet_grid(___~___)

# You can use (the labeller is usefull if you do not 
# want standard label, see `?labeller` for more informations)
# to put the variable indication before the corresponding levels you can
# use `"label_both"`
facet_grid(___~___, labeller = ___)

# For bold and italic characters, you can use "bold.italic" where 
# needed in the theme
theme(strip.text = element_text(face = "bold.italic"))
```

   
   



### Theme (stytle)

Change the plot title to "Penguins' mass and their flipper length by
years", add a caption to acknowledge that "Data from {palmerpenguins}
package.". Finally, enlarge the title to 24 points and make it orange.




```{r final-theme, exercise=TRUE}
penguins %>% 
  ggplot(aes(x = body_mass_g, y = flipper_length_mm,
    label = species, colour = sex)) +
  geom_text(check_overlap = TRUE) +
  annotate("label", x = 4500, y = 170, label = "Short",
           hjust = "inward", vjust = "inward") +
  annotate("label", x = 4500, y = 230, label = "Long",
           vjust = "inward", hjust = "inward") +
  scale_x_continuous("Body mass",
    labels = scales::label_comma(suffix = " g")) +
  scale_y_continuous("Flipper length",
    labels = scales::label_comma(suffix = " mm")) +
  scale_colour_discrete("Sex",
    breaks = c("female", "male", NA),
    labels = c("Female", "Male", "NA")) +
  facet_grid(island~year, labeller = "label_both") +
  theme(axis.title = element_text(family = "sans", colour = "blue"),
        legend.title = element_text(colour = "darkgreen"),
        strip.text = element_text(face = "bold.italic"))
```




```{r final-theme-check}
grade_result(pass_if(~{
  ref <- penguins %>% 
    ggplot(aes(x = body_mass_g, y = flipper_length_mm,
      label = species, colour = sex)) +
    geom_text(check_overlap = TRUE) +
    annotate("label", x = 4500, y = 170, label = "Short",
             hjust = "inward", vjust = "inward") +
    annotate("label", x = 4500, y = 230, label = "Long",
             vjust = "inward", hjust = "inward") +
    scale_x_continuous("Body mass",
      labels = scales::label_comma(suffix = " g")) +
    scale_y_continuous("Flipper length",
      labels = scales::label_comma(suffix = " mm")) +
    scale_colour_discrete("Sex",
      breaks = c("female", "male", NA),
      labels = c("Female", "Male", "NA")) +
    facet_grid(island~year, labeller = "label_both") +
    labs(title = "Penguins' mass and their flipper length by years",
         caption = "Data from {palmerpenguins} package.") +
    theme(plot.title = element_text(size = 24, colour = "orange"),
          axis.title = element_text(family = "sans", colour = "blue"),
          legend.title = element_text(colour = "darkgreen"),
          strip.text = element_text(face = "bold.italic"))

  test_plots(.result, ref)
  
}), incorrect = "Wrong answer.")
```



```{r final-theme-hint-1}
# You can add main titles (plot title, caption and all the title's
# legend for the aesthetics) using `labs()`
labs(title = ___,  caption = ___)
```

```{r final-theme-hint-2}
# You can add main titles (plot title, caption and all the title's
# legend for the aesthetics) using `labs()`
labs(title = ___,  caption = ___)

# You can adjust the theme of the plot title as usual
theme(___.title = element_text(___ = 24, ___ = "orange"))
```



### Greetings!!!

We agree our final plot is not nice, nor insightful... but our excuse
for that is: it was only a _pre**text**_ plot ;-)

###

You are now a master of text and labels into `{ggplot2}`.
Remember to distinguish when your "text" requirements comes from:

- **data**: map the corresponding variable to the `label` aesthetic,
  and draw them with `geom_text()` or `geom_label()` at your convenience

- **meta-data**: maybe that means you whant to manage some title or text
  in some legends (or axis, which are nothing more than the legend for
  the axes ;-). So, you can go with the corresponding `scale_<AES>_<TYPE>()`
  function (which first argument is the legend title), and you can use
  the `break`, and `label` to control which label to plot and their 
  corresponding name respectively. Or, just to change the title, you can
  use the convenient wrapper `labs()` just assigning the title you lke
  to the corresponding aesthetics (eg, labs(colour = "Colour legend")).

- **just an annotation**: so, you simply want to draw a `geom_*` for a
  single-new not-in-data point ;-)... use the `annotation()` function,
  and in its first argument set the geom you like (eg, "text" or
  "label"), nex directly assign the value you need (eg, `x`, `y`,
  `label`).

- **style**: maybe you already have some text you like to make more 
  stylish. So, you need to put a call to `theme()` and adjust the
  corresponding elements'characteristics. For text elements they are
  set calling `element_text()` for the corresponding argument
  of interest (eg, `plot.title`).
  
  
  That's all!!
  Congratulations!!
  
  Thank you to have been part of this class.
  
  ___\@CorradoLanera___


Feel free to play with penguins and `{ggplot2}` as much as you like!

```{r final-free, exercise=TRUE, exercise.lines = 5}
library(tidyverse)
library(palmerpenguins)



```







```{r quiz-template, eval=FALSE, include=FALSE}
quiz(caption = "XXX-quiz",
  question("q",
           
    answer("a",
           correct = TRUE,
           message = random_praise()),
    
    answer("b",
           message = ""),
    
    answer("c",
           message = ""),
    
    answer("d",
           message = ""),
    
    try_again = random_encouragement(),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
```



```{r parsons-template, eval=FALSE, include=FALSE}
question_parsons(
  initial = c(
    "iris",
    "mutate(...)",
    "summarize(...)",
    "print()"
  ),
  pass_if(
    c(
      "iris",
      "mutate(...)",
      "summarize(...)"
    )
  ),
  fail_if(
    ~length(.) < 2,
    message = "Include at least two answers"
  ),
  fail_if(
    function(x){"print()" %in% x},
    message = "You should not include print() in your answer"
  ),
  fail_if(
    ~{.[1] != "iris"},
    message = "Your solution should start with 'iris'"
  )
)
```


